...
- Editor dashboard `/dashboard/editorial`: the author column shows the submission's contributor name (primary contact first, then any listed contributor), falling back to the account name from `OjsLanding.User` only when the submission has no contributors (`EditorController.author_name/1`)
  ...
  - **Review Completed notification popover**: when a reviewer submitted a review (a `ReviewerAssignment` with status `:completed` matched by title), the row's EDITORIAL ACTIVITY column shows a green **email icon** (`#description .btn-review-completed`, bg `#27ae60`). Clicking it opens a fixed anchored **popover** (`.rc-popover`, id `review-completed-popover-<submission_id>-<assignment_id>`, close via X, backdrop click, or Escape) with a green badge (white text) **"Review Completed on {day Month year}"**, the reviewer username, the message **"The review was completed on {YYYY-MM-DD} with the following recommendation: {recommendation}"**, and a **"View Unread recommendation"** button (`.rc-popover-btn`, now a `<button>` with `data-drawer` = `review-details-overlay-<submission_id>-<assignment_id>`) that opens a **Review Details slide-in drawer** (`.rvd-overlay`/`.rvd-panel`, same right slide-in `translateX(100%) → 0` pattern as `.apd-*`, width `720px`): blue `#006798` header with back-arrow + centered "Review Details: {title}", reviewer username, instruction "Once this review has been read, press "Confirm"...", an info box `Completed: {YYYY-MM-DD [HH:MM]}` (from submitted_at when available), a "Reviewer Comments" section with two boxes labeled "For author and editor" / "For editor" (from `comments_author`/`comments_editor`, rendered via `raw/1`), a "Reviewer Files" section whose heading sits beside a right-aligned Search box + Upload File button with a file table (columns File | Date | Type, `data-name` rows filterable client-side by the Search input), and — below the Reviewer Files section — a "Recommendation" heading over a box containing the reviewer's recommendation (`review.recommendation`). Footer has **Cancel** / **Confirm** buttons (both close via `closeReviewDetailsDrawer(overlayId)`; Confirm currently has no server-side "read" flag so it just dismisses). Data: `to_editorial_row/1` includes `completed_reviews` (list of `%{assignment_id, reviewer, completed_on, completed_at, recommendation, comments_author, comments_editor, reviewer_files}` via `EditorController.completed_reviews/1`; completion date from `submitted_at` falling back to the last `review_history` entry's `date` — `format_completed_at/1` produces `YYYY-MM-DD HH:MM` for DateTimes, else the ISO date — recommendation from `recommendation` falling back to `review_history.decision`); formatting via `EditorHTML.review_completed_header/1` + `review_completed_message/1` + `long_date/1`; JS `openReviewCompletedPopover`/`closeReviewCompletedPopover` defined inline in `editorial.html.heex`; the Review Details drawer JS (`openReviewDetailsDrawer`/`closeReviewDetailsDrawer` + delegated `.rvd-search-input` filter + backdrop-click/Escape handlers) lives in `assets/js/app.js` (shared with the workflow page; `openReviewDetailsDrawer` guards `closeReviewCompletedPopover` via `typeof`)
...
- Editor workflow view (OJS PKP 3.5 style): `/dashboard/editorial?workflowSubmissionId=<id>&currentViewId=<view>&workflowMenuKey=<key>` opens a single submission's workflow page (`editor_html/workflow.html.heex`) instead of the submissions table
  ...
  - Assignment links ("Read Review", "Manage copyediting", "Open production record") use `.editorial-view-link` class for plain-text styling in review/copyedit/production task tables
...
- **Step 3 = Reviewer Review Page** (`review.html.heex`, rendered when `status: :in_progress and wizard_step == 3`, CSS classes with `rp-` prefix in `assets/css/app.css`, section "Reviewer Review Page (OJS 3.5 PKP style)"): a full OJS 3.5-style review interface built of stacked `.rp-panel` cards
  ...
  - **Review Discussions** panel: header with "Add Discussion" button, empty state body. The **Add Discussion** drawer (`#review-discussion-overlay`, `.prd-*`) has Participants/Subject/Message/Attached Files sections; the "Upload File" button (`#rd-btn-upload-file`) inside the Attached Files box opens an **Upload Discussion File modal** (`#review-discussion-upload-overlay`, class `urf-overlay` + `rdud-overlay` with `z-index: 10040` so it layers above the `.prd-overlay` drawer) — a right slide-in 3-step stepper (1. Upload File · 2. Review Details · 3. Confirm, JS prefix `rdud-*`). Step 1 has a required **Article Component** dropdown (Select article component / Article Text / Research Instrument / Research Materials / Research Results / Transcript / Data Analysis / Data Set / Source Text / Other); the "Upload File" button on the right only appears after a component is picked, Continue stays disabled until a file is selected, the button then becomes "Change File", and step 3 "Upload" appends the file to `rdAttachedFiles` and re-renders the custom file list (`renderReviewDiscussionAttachments`, serialized as JSON into hidden `#rd-attached-files`). The standalone "Attached Files" label outside the `.prd-attach-box` was removed (only the box's own title remains)
  - **Recommendation** panel: `<select name="recommendation" form="review-form" id="rp-recommendation">` (outside the `<form>` element, tied to it via the `form="review-form"` attribute) with a placeholder option + Accept Submission / Revisions Required / Resubmit for Review / Decline Submission — it is part of the submitted payload, so no separate recommendation step is needed
  - **Actions** panel footer: "Go back" (form POST `/review/:id/go-back`), "Save For Later" (non-functional placeholder button `rp-btn-save-later`), **"Submit Review"** (`#rp-btn-submit-review`, submits `#review-form` → **`POST /review/:id`** → `submit_review/2`). This performs the actual submission to the editor (status `:completed`, stage `:copyediting`, records `recommendation`/`comments_author`/`comments_editor` and appends to `review_history`) — the display then continues to the Step 4 Completion view: after the redirect to `/review/:id` the page renders the `:completed` "Review Submitted" summary (recommendation + comments + "Back to My Assignments") and the stepper marks Step 4 Completion as active (`wizard_current_step/%{status: :completed}` → 4)
  - The form uses a single `<form id="review-form">` (POST `/review/:id`) for the rich-text editors (each editor has a `form`-less hidden input inside the form); the recommendation `<select>` sits outside that form and is tied to it via the `form="review-form"` attribute
- Step 4 Completion wizard page (`status: :in_progress, wizard_step == 4`) still exists in the template (radio-button Recommendation + "Submit Review") but is no longer reachable from the UI — it is only entered when `advance_review_step/2` moves `wizard_step` 3→4 via `POST /review/:id/step`
- After submitting, the assignment lands on the `:completed` view showing "Review Submitted" + the recommendation/comments and the copyediting stage panels

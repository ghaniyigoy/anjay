...
- Editor workflow actions (stage transitions):
  - `POST /dashboard/editorial/:id/send-to-review` (`send_to_review/2`) — sets status `:active`, stage `:initial_review`
  ...
  - `POST /dashboard/editorial/assign-editor` (`assign_editor/2`) — appends editor info (from `OjsLanding.User`) to the submission's `editors` list via `Submission.assign_editor/2`
  - Action buttons appear in the workflow view per stage: Send For Review + Accept and Skip Review + Decline Submission in `workflow_1`; Request Revisions + Accept + Decline in `workflow_3_1`; Accept & Schedule + Decline in `workflow_5`

...
- Editor dashboard `/dashboard/editorial`: the author column shows the submission's contributor name (primary contact first, then any listed contributor), falling back to the account name from `OjsLanding.User` only when the submission has no contributors (`EditorController.author_name/1`)
  ...
  - **Editorial Activity column**: shows contextual action links based on submission state — "Assign Editor" if no editor assigned (checked first, regardless of stage, because review/editorial decisions cannot begin until an editor handles the submission), "Complete Submission" if incomplete/not yet submitted, "Assign Reviewers" if no reviewer assignments exist, or a stage-specific next action otherwise (`EditorHTML.editorial_actions/1` + `EditorHTML.next_action_for_stage/1`): `initial_review` → "Send for Review" to `workflow_3_1`, review stages → "View Reviews", `copyediting` → "Copyedit" to `workflow_4`, `production` → "Production" to `workflow_5`. Data flows through `to_editorial_row/1` which enriches each row with `has_editor`, `has_reviewers`, and `needs_submission_complete` flags derived from `Submission.editors` and `ReviewerAssignment` matches by title
  - **Assign Editor modal**: when "Assign Editor" is clicked (no editor assigned yet), `EditorHTML.editorial_actions/1` returns `%{modal: "assign-editor-#{row.id}"}` which triggers a client-side modal instead of navigating. The modal (`#assign-editor-overlay` in `editorial.html.heex`) has a header "Assign Participant", a left "Locate a User" panel with role filter dropdown (default "Journal Editor") + search input, a right "Search User By Name" table (Name, Assignments, Affiliation, Reviewing Interest columns), and a Message section with instruction text + rich text editor (bold/italic/underline/list). User data is pre-encoded as JSON (`@ap_users_json` via `Jason.encode!`) in the controller and filtered client-side. On submit, a hidden form POSTs to `POST /dashboard/editorial/assign-editor` (`EditorController.assign_editor/2`) which calls `Submission.assign_editor/2` to append the editor to the submission's `editors` list. CSS classes use `ap-` prefix (OJS 3.5 PKP design system)
- Editor workflow view (OJS PKP 3.5 style): `/dashboard/editorial?workflowSubmissionId=<id>&currentViewId=<view>&workflowMenuKey=<key>` opens a single submission's workflow page (`editor_html/workflow.html.heex`) instead of the submissions table
  - Workflow menu keys: `workflow_1` (Submission: two-column layout with Submission Files + Pre-Review Discussions on the left, Action Panel + Participants on the right), `workflow_3_1` (External Review round 1 — review assignments matched to the submission by title), `workflow_4` (Copyediting tasks), `workflow_5` (Production: galleys/proofreading). The `workflow_1` **Participants card Assign button** (`#btn-assign-participant`, `onclick="openAssignParticipantDrawer()"`) opens a right slide-in **Assign Participant drawer** (`#assign-participant-drawer`, `.apd-*` CSS, mirroring the `.ar-overlay`/`.urf-overlay` slide pattern): blue `#006798` header with back-arrow + "Assign Participant", a "Locate a User" section with a role dropdown (`#apd-role-filter`: Journal Editor / Section Editor / Guest Editor / Finding coordinator / Author / Translator) next to a search box (`#apd-search-name`) captioned "Search User By Name", and a server-rendered user list (`@ap_users` = all `OjsLanding.User.all()` via `reviewer_json/1`, no role filter) where each `.apd-user-card` shows initials avatar / name / email / role (`EditorHTML.participant_role_label/1`) + affiliation. Client-side `apdFilterUsers()` filters by role + name (`data-role` = `to_string(role)`, `data-name` = lowercase name+username+email); selecting a card adds `.apd-selected`; Send POSTs the hidden `#assign-participant-form` to `/dashboard/editorial/assign-editor` and Cancel / back-arrow / backdrop / Escape close the drawer
